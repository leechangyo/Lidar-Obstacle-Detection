...
void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer)
{
    ...
    Eigen::Vector4f minpoint (-30,-6.5,-3,1); // minimum staring region
    Eigen::Vector4f maxpoint (39,6.5,10,1); // maximum ending region
    ...
    std::pair<pcl::PointCloud<pcl::PointXYZI>::Ptr, pcl::PointCloud<pcl::PointXYZI>::Ptr> segmentCloud = pointProcessor->SegmentPlane(filterCloud,100,0.3); // cloud, itetartion, therhold distance(for ransac)
    renderPointCloud(viewer,segmentCloud.second,"planeCloud",Color(0,1,0)); // plane

    // // 2. Cluster the obstacle cloud
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudclusters = pointProcessor->Clustering(segmentCloud.first, 0.6,10, 5000); // obstacle cloud, clustertolerance, mincluster, max cluster
